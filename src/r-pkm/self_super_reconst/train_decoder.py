...
def main(argv):
    ...
    # Создаем декодер в зависимости от выбранного типа
    if FLAGS.decoder_type == 'BaseDecoder':
        dec = make_model('BaseDecoder', n_voxels, im_res(), n_chan_output=n_chan_dec_output)
    elif FLAGS.decoder_type == 'VDVAEDecoderBase':
        dec = make_model('VDVAEDecoderBase', n_voxels, n_channels=n_chan_dec_output)
        if FLAGS.vdvae_weights:
            cprintm(f'Loading VDVAE weights from {FLAGS.vdvae_weights}')
            dec.load_vdvae_weights(FLAGS.vdvae_weights)
    elif FLAGS.decoder_type == 'VDVAEDecoderRGBD':
        dec = make_model('VDVAEDecoderRGBD', n_voxels)
        if FLAGS.vdvae_weights:
            cprintm(f'Loading VDVAE weights from {FLAGS.vdvae_weights}')
            dec.load_vdvae_weights(FLAGS.vdvae_weights)
    else:
        raise ValueError(f'Unknown decoder type: {FLAGS.decoder_type}')
