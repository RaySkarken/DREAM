...
def main(argv):
    ...
    if FLAGS.encoder_type == 'SeparableEncoderVGG19ml':
        model = make_model('SeparableEncoderVGG19ml', train.n_voxels, FLAGS.random_crop_pad_percent, drop_rate=0.5)
    elif FLAGS.encoder_type == 'BaseEncoderVGG19ml':
        model = make_model('BaseEncoderVGG19ml', train.n_voxels, FLAGS.random_crop_pad_percent, drop_rate=0.5)
    elif FLAGS.encoder_type == 'VDVAEEncoderBase':
        model = make_model('VDVAEEncoderBase', train.n_voxels, FLAGS.random_crop_pad_percent, drop_rate=0.5)
        if FLAGS.vdvae_weights:
            cprintm(f'Loading VDVAE weights from {FLAGS.vdvae_weights}')
            model.load_vdvae_weights(FLAGS.vdvae_weights)
    elif FLAGS.encoder_type == 'VDVAEEncoderRGBD':
        model = make_model('VDVAEEncoderRGBD', train.n_voxels, FLAGS.random_crop_pad_percent, drop_rate=0.5)
        if FLAGS.vdvae_weights:
            cprintm(f'Loading VDVAE weights from {FLAGS.vdvae_weights}')
            model.load_vdvae_weights(FLAGS.vdvae_weights)
    else:
        raise ValueError(f'Unknown encoder type: {FLAGS.encoder_type}')
