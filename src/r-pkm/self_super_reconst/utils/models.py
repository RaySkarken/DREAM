...
def make_model(model_type, *args, **kwargs):
    ...
    if model_type == 'SeparableEncoderVGG19ml':
        return SeparableEncoderVGG19ml(*args, **kwargs)
    elif model_type == 'BaseEncoderVGG19ml':
        return BaseEncoderVGG19ml(*args, **kwargs)
    elif model_type == 'BaseDecoder':
        return BaseDecoder(*args, **kwargs)
    elif model_type == 'VDVAEEncoderBase':
        from self_super_reconst.utils.model_vdvae import VDVAEEncoderBase
        return VDVAEEncoderBase(*args, **kwargs)
    elif model_type == 'VDVAEDecoderBase':
        from self_super_reconst.utils.model_vdvae import VDVAEDecoderBase
        return VDVAEDecoderBase(*args, **kwargs)
    elif model_type == 'VDVAEEncoderRGBD':
        from self_super_reconst.utils.model_vdvae import VDVAEEncoderRGBD
        return VDVAEEncoderRGBD(*args, **kwargs)
    elif model_type == 'VDVAEDecoderRGBD':
        from self_super_reconst.utils.model_vdvae import VDVAEDecoderRGBD
        return VDVAEDecoderRGBD(*args, **kwargs)
    else:
        raise NotImplementedError(f'Model type {model_type} not found')
